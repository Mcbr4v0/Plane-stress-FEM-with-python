...
def global_stiffness_matrix(nodes,element,E,nu,t):
    ...
    for i in element:
        ...
        for j in range(3):
            for k in range(3):
                K[2*element[i][j],2*element[i][k]] += int(Ke[2*j,2*k])
                K[2*element[i][j],2*element[i][k]+1] += int(Ke[2*j,2*k+1])
                K[2*element[i][j]+1,2*element[i][k]] += int(Ke[2*j+1,2*k])
                K[2*element[i][j]+1,2*element[i][k]+1] += int(Ke[2*j+1,2*k+1])
    return K
